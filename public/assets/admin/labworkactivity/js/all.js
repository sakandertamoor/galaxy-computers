$( document ).ready( function () {
  $('.table').css({
    ...
    var path = "{{route('fetchLabWorkActivity')}}";
    $('#activities').DataTable({
           lengthMenu: [[100, 50, 25, 10, -1], [100, 50, 25, 10,  "All"]],
           processing: true,
           serverSide: true,
           ajax: allLabWorkActivity,
           columns: [
    ...
    $('#activities tbody').on( 'click', '#suspend-btn',function(){
         ...
         var activityTitle =  $(this).parents("tr").find('td.title').text();
          if(action=='suspend')
          {
            ...
            var message = 'Do you want to suspend Lab Work Activity';
          }else{
         // var message = 'Are you sure want to activate '+activityTitle+' activity';
         var message = 'Do you want to re-active Lab Work Activity';
          }
         Swal.fire({
          title: 'Are you sure?',
          text:message,
